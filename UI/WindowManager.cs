...
{
    /// <summary>
    /// The in-game overlay: a small windowing layer for the "Overview" (the list that toggles every mod's panel),
    /// each mod's panel, and the log "Timeline". Each window is independently toggleable, movable (drag the title
    /// bar) and resizable (drag the bottom-right grip), with a scrollable body and persisted layout. Rendering uses
    /// cached-rebuild IMGUI; interaction uses polled legacy <see cref="Input"/> (proven in this IL2CPP build, where
    /// Event.current mouse input is not) - so buttons/toggles are hit-tested against the rects drawn last frame.
    /// <see cref="Draw"/> runs from OnGUI, <see cref="HandleInput"/> from OnUpdate.
    /// </summary>
    internal static class WindowManager
    {
        ...
        private static void RefreshWindowList()
        {
            ...
            {
                WindowLayout.Get(panels[i].Id, 360f + (i % 3) * 28f, 40f + i * 34f, 320f, 240f, false);
        ...
        private static void DrawBody(string id, float innerW, float scroll, Rect bodyRect, ref float localY)
        {
            ...
            if (kind == "overview")
            {
                Label(innerW, scroll, bodyRect, ref localY, "<b>Hotline</b>  -  toggle a mod's panel");
                IReadOnlyList<PanelModel> panels = PanelRegistry.All;
                if (panels.Count == 0)
                    Label(innerW, scroll, bodyRect, ref localY, "<color=#8a8f9e>no mod panels registered yet</color>");
                for (int i = 0; i < panels.Count; i++)
        ...
        // accent: 0 = neutral, 1 = green, 2 = red. on = toggle active (violet).
